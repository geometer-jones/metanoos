...
class ComposedStateMixing(nn.Module):
        ...
        *,
        ...
        if head_dim is None:
            if d_model % num_heads != 0:
                raise ValueError("d_model must be divisible by num_heads when head_dim is not provided")
        ...
        self.q_proj = ComplexLinear(d_model, self.inner_dim)
        self.k_proj = ComplexLinear(d_model, self.inner_dim)
        self.v_proj = ComplexLinear(d_model, self.inner_dim)
        self.out_proj = ComplexLinear(self.inner_dim, d_model)
    ...
    def _merge_heads(self, x: Tensor) -> Tensor:
        return x.reshape(*x.shape[:-2], self.inner_dim)
    ...
    def forward(self, x: Tensor, *, return_state: bool = False) -> Tensor | tuple[Tensor, AssociativeState]:
        ...
        out = self.out_proj(self._merge_heads(y))
        if return_state:
            return out, prefix
        return out
    ...
    def step(self, x: Tensor, memory: AssociativeState | None = None) -> tuple[Tensor, AssociativeState]:
        ...
        return self.out_proj(self._merge_heads(y)), new_memory
